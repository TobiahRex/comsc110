...
int main() {
  ...
  do {
    printMenu();
    int rand1(0), rand2(0), r1Length(0), r2Length(0), solutionLength(0), wWidth(0);
    float solution(0);
    srand(time(NULL));
    wWidth = 11;
    rand1 = rand() % 1000;
    ...
    switch(choice) {
      ...
      case 4: {
        ...
        float div_solution = rand1 / rand2;
        int div1 = to_string(dividend).length();
        int div2 = to_string(divisor).length();
        solutionLength = to_string(div_solution).length();

        cout << setw(wWidth - div2) << dividend << endl;
        cout << "\u00F7" << setw(wWidth - div2 - 1) << divisor << endl;
        cout << lineBreak << endl;
        cout << setw(wWidth - div2);
